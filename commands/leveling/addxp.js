...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!args[0]) return message.reply({
            content: `${process.env.FAILURE_EMOJI} Please mention a user that you want to give xp`,
            
        })
        if (!args[1]) return message.reply({
            content: `${process.env.FAILURE_EMOJI} Please tell the amount of xp to add!`,
        ...
        Levels.appendXp(user, message.guild.id, xpamount)

        message.reply({
            content: `${process.env.SUCCESS_EMOJI}I have added ${args[1]} xp to <@${target.id}>`,
