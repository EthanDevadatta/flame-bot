...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        if (SubCommand === "channel") {
            ...
            Schema.findOne({
                ...
                if (data) {
                    const embed = new MessageEmbed()
                        ...
                        .setTimestamp()
                        .setFooter({text:`Suggestion Token: ${pass}`})
                    channell.send({
                        embeds: [embed]
                    }).then(m => {
                        const replyembed = new MessageEmbed()
                            .setColor('RED')
                            .setDescription(`${process.env.SUCCESS_EMOJI} Your [Suggestion](https://discord.com/channels/${interaction.guild.id}/${data.Channel}/${m.id}) has been sent!`)
                        interaction.followUp({
                            embeds: [replyembed]
                        })
                        m.react('<a:upvote:767586020027531314>')
                        m.react('<a:downvote:767586020010754078>')
                        new CodeSchema({
                            message: m.id,
                            token: pass,
            ...
            if (!interaction.member.permissions.has("MANAGE_GUILD")) return interaction.followUp({ content: "You do not have proper permissions!" })
            ...
            Schema.findOne({
                ...
                if (channel) {
                    const embed = new MessageEmbed()
                        ...
                        .setTimestamp()
                        .setFooter({text:`Suggestion Token: ${stoken}`})
